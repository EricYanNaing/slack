...
import { Channel, Workplace } from "@/types/app";
...
const SearchBar: FC<SearchBarProps> = ({
  ...
  const addUserToChannel = async (userId: string, channelId: string) => {
    // Add user to channel
    await updateChannelMembers(userId, channelId);
  ...
  return (
    <div
      className={cn(
        "absolute h-10 w-[500px] px-3 top-2 rounded-md bg-slate-400"
      )}
    >
      <Popover>
        ...
        <PopoverContent className="w-[500px]">
          <ScrollArea className="rounded-md max-h-96">
            {currentWorkplaceData?.members?.map((member) => {
              return (
                <div
                  key={member.id}
